...
namespace Game
{
    ...
    public abstract class Enemy : AbstractRole, IEnemy
    {
        ...
        [ShowInInspector]
        public List<PathFindingHelper.NodeBase<Vector3Int>> MovementPath = new();
        ...
        protected override void Awake()
        {
            ...
            Property.Hp.Register((oldValue, value) =>
            {
                if (value <= 0)
                {
                    ...
                    var dieBody = FxFactory.PlayDieBody(this.GetPosition(), GetType().Name.Substring(0, 6) + "Die", _lastHitInfo,
                        SpriteRenderer.GetLocalScaleX());

                    foreach (Transform child in transform)
                    {
                        if (child.name == "HalfArrow")
                        {
                            child.SetParent(dieBody);
                        }
                    }
